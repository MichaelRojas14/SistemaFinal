...
@Component({
  selector: 'app-ingresar-res-juv',
  templateUrl: './ingresar-res-juv.component.html',
  styleUrls: ['./ingresar-res-juv.component.css']
})
export class IngresarResJuvComponent implements OnInit {
  listaClub: Club[] = [];
  listaSerie: Serie[] = [];
    ...
    if(this.id !== null){
      ...
      this._partserieService.updatePartSerie(this.id, PART_SERIE).subscribe({
        next: resp => {
          ...
          this.router.navigate(['/resultados-admin']);
