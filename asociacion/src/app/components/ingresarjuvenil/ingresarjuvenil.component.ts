...
@Component({
  selector: 'app-ingresarjuvenil',
  templateUrl: './ingresarjuvenil.component.html',
  styleUrls: ['./ingresarjuvenil.component.css']
})
export class IngresarjuvenilComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    { 
      ...
      this.id = this.aRouter.snapshot.paramMap.get('id_tabla_juv')!;
    }
  ...
  agregarPosicion() {
    ...
    if(this.id !== null){
      //Editar Club
      this._juvenilService.editarPosicionJuv(this.id, JUVENIL). subscribe(data =>{
        this.toastr.info('La Posicion fue actualizada con exito','Posicion Actualizado');
        this.router.navigate(['/tabla-juvenil-admin']);
      }, error => { 
        console.log(error);
      ...
      //Agregar Club
      console.log(JUVENIL);
    this._juvenilService.guardarPosicion(JUVENIL).subscribe(data => {
  ...
  esEditar(){
    if(this.id !== null){
      ...
      this._juvenilService.obtenerPosicion(this.id).subscribe({
        next: (data: any) => {
