...
@Component({
  selector: 'app-ingresar-primera',
  templateUrl: './ingresar-primera.component.html',
  styleUrls: ['./ingresar-primera.component.css']
})
export class IngresarPrimeraComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder,
    ...
    { 
      ...
      this.id = this.aRouter.snapshot.paramMap.get('id_tabla_prim')!;
    }
  ...
  agregarPosicion() {
    ...
    if(this.id !== null){
      //Editar Club
      this._primeraService.editarPosicion(this.id, PRIMERA). subscribe(data =>{
        this.toastr.info('La Posicion fue actualizada con exito','Posicion Actualizado');
        this.router.navigate(['/tabla-primera-admin']);
      }, error => { 
        console.log(error);
      ...
      //Agregar Club
      console.log(PRIMERA);
    this._primeraService.guardarPosicion(PRIMERA).subscribe(data => {
  ...
  esEditar(){
    if(this.id !== null){
      ...
      this._primeraService.obtenerPosicion(this.id).subscribe({
        next: (data: any) => {
